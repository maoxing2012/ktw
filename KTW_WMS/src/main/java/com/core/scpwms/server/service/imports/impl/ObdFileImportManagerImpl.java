...
import com.core.scpwms.server.service.imports.ObdFileImportManager;
 ...
 *
 */
public class ObdFileImportManagerImpl extends DefaultBaseManager implements ObdFileImportManager {
	// 日本食研
	private ObdFileImportWithTransManager nsObdFileImportWithTransManager;
	
	@Override
	public Map<String, String> importNs(Long id) {
		return importFile(id, nsObdFileImportWithTransManager);
	}

	@Override
	public Map<String, String> importNsForce(Long id) {
		return importFileForce(id, nsObdFileImportWithTransManager);
	}
	
	public ObdFileImportWithTransManager getNsObdFileImportWithTransManager() {
		return nsObdFileImportWithTransManager;
	}

	public void setNsObdFileImportWithTransManager(
			ObdFileImportWithTransManager nsObdFileImportWithTransManager) {
		this.nsObdFileImportWithTransManager = nsObdFileImportWithTransManager;
	}
	...
	private Map<String, String> importFile( Long id, ObdFileImportWithTransManager obdImportManager ){
		Map<String, String> result = new HashMap<String, String>();
		FileImport fileInfo = getCommonDao().load(FileImport.class, id);

		if (fileInfo != null) {
			String[][] csvFile = FileUtil4Jp.readCSVFile(fileInfo.getTargetName(), FileUtil4Jp.ENCODE_MS932, obdImportManager.getCsvPreference());
			...
			try{
				...
				results = obdImportManager.createObd(csvFile, resultList, fiw.getOwner(), Boolean.FALSE);// 正常
			...
			if( resultList != null && resultList.size() > 0 ){
				...
				// 处理结果信息
				fileInfo.setWarnningMessage("インポートに成功しました。" + results[0]);
	...
	private Map<String, String> importFileForce( Long id, ObdFileImportWithTransManager obdImportManager ){
		Map<String, String> result = new HashMap<String, String>();
		FileImport fileInfo = getCommonDao().load(FileImport.class, id);
		...
		if (fileInfo != null) {
			...
			try{
				String hql = " from FileImport4Wms fiw where fiw.fileImport.id = :fiId ";
				FileImport4Wms fiw = (FileImport4Wms)commonDao.findByQueryUniqueResult(hql, new String[]{"fiId"}, new Object[]{id});
				results = obdImportManager.createObd(csvFile, resultList, fiw.getOwner(), Boolean.TRUE);// 强制导入
			}catch(BusinessException be){
				resultList.add( new String[]{ "-1", be.getLocalizedMessage() } );
			}
