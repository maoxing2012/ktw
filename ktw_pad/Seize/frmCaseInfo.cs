    ...
    public partial class frmCaseInfo : Framework.BaseForm
    {
        ...
        public override int Init(bool back, params object[] paras)
        {
            ...
            if (!back)
            {
                ...
                if (ret != 0)
                    return ret;
                lblTitle.Text = "出荷検品（{0}）".FormatEx(AppContext.Instance().Owner.name);
            }
            ...
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    if (StringUtil.IsEmpty(txtBarCode.Text))
                    {
                        ...
                        txtBarCode.Focus();
                        Framework.Message.Warn("商品バーコードを入力してください");
                        return;
                    }
                    if (txtCaseNumber.Text.Equals(txtBarCode.Text))
                    {
                        txtBarCode.SelectAll();
                        txtBarCode.Focus();
                        Framework.Message.Warn("検品数不足、検品完了できません。");
                        return;
                    }
                    ...
                    if (ret.ActionResult == ActionResult.SelectOne)
                    {
                        ...
                        if (existFlg == 0)
                        {
                            Framework.Message.Warn("個口に該当商品がありません。");
                            txtBarCode.SelectAll();
                            txtBarCode.Focus();
                        }
                        else
            ...
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    if (StringUtil.IsEmpty(txtCaseNumber.Text))
                    {
                        ...
                        txtCaseNumber.Focus();
                        Framework.Message.Warn("個口番号を入力してください");
                        return;
                    }
        ...
        private void F1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                lblInfo.Visible = false;
                //清空控件
                this.ClearControl(true, txtCaseNumber, txtBarCode,  lblCaseQty, lblQty, lblSkuCode, lblSkuName, lblUnit, lblNum, lblSpecs, lblExpDate);
                txtCaseNumber.Focus();
                txtCaseNumber.SelectAll();
                ...
                if (inCaseCount == caseCount)
                {
                    try
                    {
                        ...
                        if (!txtBarCode.Text.Equals(txtCaseNumber.Text))
                        {
                            txtBarCode.SelectAll();
                            txtBarCode.Focus();
                            Framework.Message.Warn("個口番号は一致していいですか。");
                            return;
                        }
                        else
                        {
